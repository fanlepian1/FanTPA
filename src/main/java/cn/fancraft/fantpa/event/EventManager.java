...
public class EventManager {
    ...
    public <T> void fire(T event) {
        ...
        for (EventListener<?> l : list) {
            try { ((EventListener<T>) l).onEvent(event); }
            catch (Exception e) { LoggerUtil.error("事件处理异常: " + event.getClass().getSimpleName(), e); }
        }
    }
    ...
    public <T> CompletableFuture<Void> fireAsync(T event) {
        return CompletableFuture.runAsync(() -> {
            ...
            for (EventListener<?> l : list) {
                try { ((EventListener<T>) l).onEvent(event); }
                catch (Exception e) { LoggerUtil.error("异步事件处理异常: " + event.getClass().getSimpleName(), e); }
            }
        });
    }
